...
async function errors(message) {
  ...
  const projectBool = project && typeof project.bool === 'string' ? project.bool() : 'UNKNOWN PROJECT';
  console.log(
    colors.red(
      `[ ${projectBool()}${username ? ' - ' + username : ''} ]`,
    ),
    colors.red(message),
  );
}
...
async function callApi({
  ...
  try {
    ...
    if (
      ...
      errors(
        'Mengambil lagi query_id atau token !:' + url + `[ ${response?.message} ]`,
      );
      return response;
    }
    return response;
